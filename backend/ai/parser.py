...
Ты помощник по структуре афиш мероприятий.
    ...
    Приведи дату в формате ДД.ММ.ГГГГ, например 19 марта будет 19.03.2025.
    Если год не указан, используй текущий год 2025.
    Age limit добавляй без +, то есть если на афише ты видишь 12+,
    то просто 12, так как это int значение.
    ...
    if not json_match:
        return {
            k: None for k in
            ["title", "description", "date", "time", "place", "age_limit", "event_type"]
        }
